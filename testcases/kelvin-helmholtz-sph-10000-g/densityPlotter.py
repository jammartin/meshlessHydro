...
# decorator function
def profile(createPlot):
    def wrapper(*args, **kwargs):
 
        mem_before = process_memory()
        result = createPlot(*args, **kwargs)
...
def createPlot(h5File, outDir, plotGrad, plotVel, iNNL):
	...
	with h5.File(h5File, 'r') as data:
		...
		rhoPlt = ax.scatter(pos[:,0], pos[:,1], c=rho, s=230.) # good for 10**4 particles
...
def createPressurePlot(h5File, outDir):
    ...
    PPlt = ax.scatter(pos[:,0], pos[:,1], c=P, s=20.) # good for 10**4 particles
